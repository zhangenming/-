...
const DEFAULT_ACCOUNTANT_SETTLEMENT_RATIO = 50;
const DEFAULT_EXISTING_ACCOUNTANT_SETTLEMENT_RATIO = DEFAULT_ACCOUNTANT_SETTLEMENT_RATIO;
const DEFAULT_NEW_ACCOUNTANT_SETTLEMENT_RATIO = DEFAULT_ACCOUNTANT_SETTLEMENT_RATIO;
...
function normalizeDateOnlyValue(value) {
  ...
  if (!source) return "";
  const structured = parseStructuredDateTimeValue(source);
  if (structured) return structured.formattedDate;
  const date = new Date(source);
  if (Number.isNaN(date.getTime())) return "";
